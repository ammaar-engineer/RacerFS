...
describe('Payment Workflow - Buy Storage', () => {
  ...
  describe('Step 2: Beli storage 100MB+', () => {
    it('Berhasil beli storage', async () => {
      const res = await request(app.getHttpServer())
        .post('/payments/buy-storage')
        .set('authorization', testAccountToken);
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
  ...
  describe('Step 3: Cek storage info setelah beli', () => {
    it('Storage bertambah sebesar 100MB (104857600 bytes)', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.data.total_storage).toBe(
        initialStorage.total_storage + expectedIncrease,
      );
      ...
      expect(res.body.data.available_storage).toBe(
        initialStorage.available_storage + expectedIncrease,
      );
    });
  });
});
