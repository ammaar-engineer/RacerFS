...
export class SnippetServicesClass {
    ...
    async createSnippet(alias, command, description) {
        try {
            const token = this.getAuthToken();
            const response = await axios.post(`${BACKEND_URL}/snippet/create`, { alias, command, description }, { headers: { authorization: token } });
    ...
    async updateSnippet(alias, command) {
        try {
            const token = this.getAuthToken();
            const response = await axios.patch(`${BACKEND_URL}/snippet/edit?alias=${encodeURIComponent(alias)}`, { command }, { headers: { authorization: token } });
            return response.data.data.snippet;
        }
        catch (error) {
    ...
    async deleteSnippet(alias) {
        try {
            const token = this.getAuthToken();
            await axios.delete(`${BACKEND_URL}/snippet/delete?alias=${encodeURIComponent(alias)}`, { headers: { authorization: token } });
        }
        catch (error) {
            if (error.response?.status === 404) {
                console.log(chalk.red(`Snippet '${alias}' not found`));
...
//# sourceMappingURL=snippet.services.js.map